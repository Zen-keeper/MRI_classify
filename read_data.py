...
def process_img(path1,path2,numofdata):
    ...
    #     # batch_res.append(read_singleflip(x_paths[i], "path",1))
    #     # batch_res.append(read_singleflip(x_paths[i], "path",1))
    ...
    #     batch_test.append(read_single(x_train_paths[i],"path"))
    #     y_test.append(y_test_paths[i])
    #
    # y_test = np.array(y_test)
    # # y = (np.arange(2) == y_res[:, None]).astype(int)
    return x_train_paths, x_test_paths, y_train_paths, y_test_paths
