...
class Train(Build_model):
    ...
    def start_train(self):
        ...
        k = 1
        # K折交叉验证
        for cnt, (train, valid) in enumerate(skf.split(X_train,dataset.y_train)):
            # if(config.case!="combineT"):
            #     X_train = []
            #     y_res = []
            #     for i in train:
            ...
            for count in range(1):
                ...
                self.train(np.array(X_train[train]), np.array(X_train[valid]), y_train[train], y_train[valid],model,k)
                self.try_on_v(model,X_train[valid],y_train[valid],k)
