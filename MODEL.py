...
class MODEL(object):
    ...
    def TSL16(self):
        ...
        model.add(Dropout(0.5))
        model.add(Dense(self.config.classNumber,activation='sigmoid'))
        return model
    ...
    def AlexNet(self):
        ...
        input_shape = (self.config.normal_size, self.config.normal_size, self.config.channles)
        model.add(
            Convolution2D(96, (7, 7), input_shape=input_shape, strides=(4, 4), padding='valid', activation='relu',
        ...
        model.add(
            Convolution2D(384, (3, 3), strides=(1, 1), padding='same', activation='relu', kernel_initializer='uniform'))
        model.add(
            Convolution2D(256, (3, 3), strides=(1, 1), padding='same', activation='relu', kernel_initializer='uniform'))
        model.add(MaxPooling2D(pool_size=(2, 2), strides=(2, 2)))
        model.add(Flatten())
        # model.add(Dense(4096, activation='relu'))
...
class ResnetBuilder(object):
    ...
    def build(self,config, block_fn, repetitions):
        ...
        conv1 = self._conv_bn_relu(filters=64, kernel_size=(3, 3), strides=(2, 2))(input)
        ...
        # flatten1 = GaussianNoise(0.1)(flatten1)
        dense = Dense(units=num_outputs,
                      activation="softmax",kernel_regularizer=regularizers.l2(0.001))(flatten1)
        # dense = Dense(units=num_outputs,
        #               activation="relu", kernel_regularizer=regularizers.l2(0.001))(flatten1)
